...
file_pattern = '/mnt/c/Users/moonm/REU 24/myenv2/lib/python3.10/site-packages/NuRadioReco/examples/cr_data/cr_16.5eV_5deg*.nur'
...
for event in nuradioreco_io.get_events():
  #print(event)
    ...
    #Vmax = np.max(all_traces)
    #Vmin = np.min(all_traces)
    #dV = Vmax - Vmin
    Vpp.append(dV)
    ...
    plt.title("Voltage Trace vs Corresponding Time(10^16eV and 5 deg zenith)")
    outfile = str("eff_area_calc_18.5eV_5zenith/trace_test_ev_noise_%s.pdf" % event_num)
    plt.savefig(outfile)
    plt.clf()
    plt.close()"""
    #break
...
"""
for i, rms in enumerate(Vrms):
    print(f"Event {i+1}: Vrms = {rms}")"""

for i, SNR_val in enumerate(SNR):
    print(f"Event {i+1}: SNR = {SNR_val}")
...
plt.title("Trigger Efficiency(16.5eV_5deg) vs Average SNR")
plt.savefig("Trigger_Efficiency_16.5eV_5deg.pdf")
