...
class tri_model(medium_base.IceModel):
    def __init__(self):
        if not medium_base.radiopropa_is_imported:
            logger.error('This ice model depends fully on RadioPropa, which was not import, and can therefore not be used.'+
                         '\nMore info on https://github.com/nu-radio/RadioPropa')
        ...
        super().__init__(z_bottom = -3000*units.meter)
        self.z_firn1 = -20.5*units.meter
        self.z_firn2 = -102*units.meter
        self._z1 = 1/0.022
        self._z2 = 1/0.0195
        self._z3 = 1/0.033
        
    def get_index_of_refraction(self, position):
        if position[2]>self.z_firn2:
            scalarfield = RP.IceModel_Firn(
                z_surface = self.z_air_boundary*RP.meter/units.meter,
                z_firn = self.z_firn1*RP.meter/units.meter, 
                n_ice = 1.78,  
                delta_n = 0.45*np.exp(self.z_firn1/self._z1),  
                z_0 = self._z2*RP.meter,
                z_shift = self.z_firn1*RP.meter,
                n_ice_firn = 1.78,
                delta_n_firn = 0.45, 
                z_0_firn = self._z1*RP.meter,
                z_shift_firn = 0.*RP.meter,
                )
        else:
            scalarfield = RP.IceModel_Firn(
                z_surface = self.z_air_boundary*RP.meter/units.meter,
                z_firn = self.z_firn2*RP.meter/units.meter, 
                n_ice = 1.78,  
                delta_n = 0.45*np.exp((self.z_firn2-self.z_firn1)/self._z2)*np.exp(self.z_firn1/self._z1),  
                z_0 = self._z3*RP.meter,
                z_shift = -self.z_firn2*RP.meter,
                n_ice_firn = 1.78,
                delta_n_firn = 0.45*np.exp(-self.z_firn1/self._z1), 
                z_0_firn = self._z2*RP.meter,
                z_shift_firn = self.z_firn1*RP.meter,
                )            
        
        #print('pos tri',position)
        position = RP.Vector3d(*(position * RP.meter/units.meter))
        #print(scalarfield.getValue(position))
        return scalarfield.getValue(position)

    def get_average_index_of_refraction(self, position1, position2):
        #taking lower z value (greater depth) of the two positions
        lowest = np.minimum(position1[2],position2[2])
        if lowest>self.z_firn2:
            scalarfield = RP.IceModel_Firn(
                z_surface = self.z_air_boundary*RP.meter/units.meter,
                z_firn = self.z_firn1*RP.meter/units.meter, 
                n_ice = 1.78,  
        ...
        position1 = RP.Vector3d(*(position1 * RP.meter/units.meter))
        position2 = RP.Vector3d(*(position2 * RP.meter/units.meter))
        return scalarfield.getAverageValue(position1, position2)
    ...
    def get_gradient_of_index_of_refraction(self, position):
        ...
        pos = RP.Vector3d(*(position * RP.meter/units.meter))
        return scalarfield.getGradient(pos) * (1 / (units.meter/RP.meter))
    ...
    def get_ice_model_radiopropa(self):
        ...
        return medium_base.RadioPropaIceWrapper(self, scalarfield)
